...
#define Vector2i_RIGHT Vector2i(1, 0)
	...
	struct tilemap {
		...
		static constexpr int BYTES_NEEDED =
			WIDTH * HEIGHT / 8
			+ (((WIDTH * HEIGHT) % 8) == 0 ? 0 : 1);		// It adds 1 if there's remainder since "/" rounds down
	...
	struct player_action_queue {
		...
		void consume_next();
		std::pair<player_entity::action, uint32> read_next();			// Reads oldest value and consumes it
		void append_new(player_entity::action action, uint32 tick);		// Adds new to the last index, removing the oldest element to make space (if full)
	...
	struct game {
		...
		mutable bool m_queue_game_start = false;					// If the match should start next send tick
#endif

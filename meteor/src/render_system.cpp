...
namespace meteor::render{
	...
	void client_system(const uint32& tick, const game& game_instance, const connection& conn, const Texture& texture) {
		...
		// ==== STATISTICS ====
		{
			...
			render_rtt_graph(MAP_OFFSET + Vector2(120, MAP_SIZE.y + 200), conn, -500.0f);
			render_send_bytes_hist(MAP_OFFSET + Vector2(240, MAP_SIZE.y + 200), conn, -0.5f);
			render_recieve_bytes_hist(MAP_OFFSET + Vector2(360, MAP_SIZE.y + 200), conn, -0.5f);
